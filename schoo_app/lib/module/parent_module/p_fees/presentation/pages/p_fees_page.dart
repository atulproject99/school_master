...
class PFeesPage extends HookConsumerWidget {
  PFeesPage({Key? key, required this.studentId, required this.nextDueDate})
      : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future.delayed(const Duration(seconds: 1), () {
      ...
      var fine = studentFees.value?.fines?.toDouble() ?? 0;
      _totalAmountController.text = " ${feesAmount + lastDue + fine}";
      /*  totalAmpunt.value =
    ...
    ref.watch(studentFeesProvider(studentId)).when(
        data: (data) {
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            ref.watch(studentFeesProvider(studentId)).when(data: (data) {
              return ListView.builder(
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  itemCount: data.monthname?.length ?? 0,
