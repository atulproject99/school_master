...
class AddStudyMaterialPage extends HookConsumerWidget {
  const AddStudyMaterialPage({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen<AddStudyMaterialState>(studyMaterialProvider, (_, state) {
      state.maybeMap(
        orElse: () {},
        success: (_) {
                ...
                child: AppButton(
                    ...
                    callback: () {
                      if (session.value.isEmpty ||
                        ...
                        return;
                      } else if (pdfFile.value == null) {
                        AppSnackBar.showInSnackBar(
                            "Please choose pdf", context);
                        return;
                      }

                      ref.read(studyMaterialProvider.notifier).addStudymaterial(
                          session.value,
                          classId.value,
                          subjectId.value,
                          pdfFile.value,
